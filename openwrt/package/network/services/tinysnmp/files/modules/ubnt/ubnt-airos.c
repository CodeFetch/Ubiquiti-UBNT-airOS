...
#define set_str_value(idx, val) do { \
	size_t str_length = strlen(val); \
	str.len = str_length < sizeof(val) ? str_length : sizeof(val); \
	str.buf = (unsigned char*) val; \
	set_value(idx, BER_OCTET_STRING, &str); \
} while (0)
...
static int update_wlan_stats(struct odb **odb, const airos_stats_t* stats) {
	...
	int rate = st->tx_rate * 1000*1000;
	set_value(9, BER_INTEGER, &rate);
	rate = st->rx_rate * 1000*1000;
	set_value(10, BER_INTEGER, &rate);

	set_str_value(11, st->security);
...
static int update_stalist_stats(struct odb **odb, const airos_stats_t* stats) {
	...
	for (i = 0; i < stats->sta_list.count; ++i) {
		...
		set_value(17, BER_INTEGER, &sta->tx_capacity);
		set_value(18, BER_INTEGER, &sta->rx_capacity);

		set_value(19, BER_INTEGER, &sta->tx_airtime);
		set_value(20, BER_INTEGER, &sta->rx_airtime);
		set_value(21, BER_INTEGER, &sta->tx_latency);
	}

	return 0;
}

static int update_host_stats(struct odb **odb, const airos_stats_t* stats) {
	const host_stats_t* st = &stats->host_stats;
	octet_string_t str;
	char tmp_time[20] = { 0 };
	int tmp_load;

	uint32_t table[] = { 11, 43, 6, 1, 4, 1, 41112, 1, 4, 8, 0, 0 };
	const uint8_t idx_col = 10;

	log_printf(LOG_VERBOSE, "updating host info\n");

	strftime(tmp_time, sizeof(tmp_time), "%F %T", localtime(&st->time.tv_sec));
	tmp_load = (int)(((float)st->cpuload * 100));

	set_str_value(1, tmp_time);
	set_value(2, BER_INTEGER, &st->netrole);
	set_value(3, BER_INTEGER, &tmp_load);
	set_value(4, BER_INTEGER, &st->sys_temperature);

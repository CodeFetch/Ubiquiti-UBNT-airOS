...
#if defined(CONFIG_AR7242_S16_PHY) || defined(CONFIG_ATHRS17_PHY) || defined(CONFIG_AR7242_8035_PHY) //KEBA
    if (is_wasp() && mac->mac_unit == 0) {
        mgmt_cfg_val = 4;
#ifdef CONFIG_AR7242_S16_PHY
        printf("WASP  ----> S16 PHY *\n");
#elseif CONFIG_ATHRS17_PHY
        printf("WASP  ----> S17 PHY *\n");
#else
#ifndef ETH_DRV_SILENT
        printf("WASP  ----> PHY Auto *\n");
#endif
        /* For UBNT we are using 0x6 instead of 0x4.
         * MDC is too high with 0x4.
         */
        mgmt_cfg_val = 6;
#endif
        if(mac->mac_unit == 0)

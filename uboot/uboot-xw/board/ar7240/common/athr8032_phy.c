...
athr8032_configure_gpio(void){
	...
	ar7240_reg_wr(0x19000004, (ar7240_reg_rd(0x19000004) & ~AG7240_MAC_CFG2_IF_1000) | (AG7240_MAC_CFG2_PAD_CRC_EN | AG7240_MAC_CFG2_LEN_CHECK | AG7240_MAC_CFG2_IF_10_100));
...
athr8032_phy_probe(int unit) {
   ...
   if (!found) {
		/*UBNT: this hack reconfigures CPU for AR8032 support if no AR8035 found*/
		if(!athr8035_found()) {
			athr8032_configure_gpio();
		}
      found = !!athr8032_phy_find(unit);
      if (found) { 
		DRV_PRINT("AR8032 Detected\n");
		athr8032_phy_found = 1;
      }
   }
   return found;
}
...
//UBNT: this is causes problems on 10MB link
//   phy_reg_write(unit, phy->phy_addr, 0x7, 0x2001);// NB: undocumented?
   phy_reg_write(unit, phy->phy_addr, ATHR_INTERRUPT_ENABLE, 0x0); // Disable all interrupts in u-boot driver
//UBNT: this is causes problems on 10MB link
//   phy_reg_write(unit, phy->phy_addr, ATHR_PHY_SPEC_CONTROL, phy_reg_read(unit, phy->phy_addr, ATHR_PHY_SPEC_CONTROL) | (1 << 11));

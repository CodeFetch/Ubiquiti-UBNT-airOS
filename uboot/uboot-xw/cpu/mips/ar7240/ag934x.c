...
int ag7240_miiphy_read_ex (char *devname, uint32_t phaddr, uint8_t reg, uint16_t* value);
int ag7240_miiphy_write_ex(char *devname, uint32_t phaddr, uint8_t reg, uint16_t  value);

uint16_t ag7240_miiphy_read(char *devname, uint32_t phaddr, uint8_t reg) {
   uint16_t value = 0;
   return (ag7240_miiphy_read_ex(devname,phaddr,reg,&value) == 0) ? value : 0;
}

void ag7240_miiphy_write(char *devname, uint32_t phaddr, uint8_t reg, uint16_t value) {
   ag7240_miiphy_write_ex(devname, phaddr, reg, value);
}
...
#ifdef CONFIG_ATHR8236_PHY
extern void athr8236_reg_init(void);
extern void athr8236_reg_init_lan(void);
extern int athr8236_mdc_check(void);
#endif
...
#ifdef CONFIG_ATHR8032_PHY
int athr8032_phy_is_up(int macUnit);
int athr8032_phy_is_fdx(int macUnit);
int athr8032_phy_speed(int macUnit);
u32 athr8032_phy_get_xmii_config(int macUnit, u32 speed);
int athr8032_probe(int macUnit);
int athr8032_phy_setup(int macUnit);
#endif
...
#if defined(CONFIG_AR7242_S16_PHY) || defined(CONFIG_ATHRS17_PHY) || defined(CONFIG_ATHR8035_PHY) || defined(CONFIG_ATHR8032_PHY) || defined(CFG_ATHRS26_PHY)
    if (is_wasp() && mac->mac_unit == 0) {
#ifdef  ET_DEBUG
   #if defined(CONFIG_AR7242_S16_PHY)
        printf("WASP  ----> S16 PHY *\n");
   #elif defined(CONFIG_ATHRS17_PHY)
        printf("WASP  ----> S17 PHY *\n");
   #elif defined(CONFIG_ATHR8035_PHY)
        printf("WASP  ----> 8035 PHY *\n");
   #elif defined(CONFIG_ATHR8032_PHY)
        printf("WASP  ----> 8032 PHY *\n");
   #elif defined(CFG_ATHRS26_PHY)
        printf("WASP  ----> AR8236 PHY *\n");
   #endif
#endif  /* ET_DEBUG */
...
#endif  /* #if defined(CONFIG_AR7242_S16_PHY) || defined(CONFIG_ATHRS17_PHY) || defined(CONFIG_ATHR8035_PHY) || defined(CONFIG_ATHR8032_PHY) || defined(CFG_ATHRS26_PHY) */
...
static void ag7240_hw_start(ag7240_mac_t *mac)
{
    ...
    /* use cfg5 value from Linux driver - accept broadcasts */
    /*
...
static int ag7240_check_link(ag7240_mac_t *mac)
{
    ...
    if(!mac->link) {
#ifdef ET_DEBUG
        printf("%s: %s link down, exiting.\n", __FUNCTION__, mac->dev->name);
#endif // #ifdef ET_DEBUG
        return 0;
    }
    ...
    switch (speed)
    {
         case _1000BASET:
          ...
          if (is_wasp() && mac->mac_unit == 0 && is_f1e() ) {
              ...
              ar7240_reg_rmw_set(AG7240_ETH_CFG,AG7240_ETH_CFG_RXDV_DELAY);
          }
          break;
       ...
       case _100BASET:
           ...
           if (is_wasp() && mac->mac_unit == 0 && is_f1e()) {
                ...
                ar7240_reg_rmw_clear(AG7240_ETH_CFG,AG7240_ETH_CFG_RXDV_DELAY);
           }
           break;
...
static void ag7240_get_ethaddr(struct eth_device *dev)
{
    ...
    if (mac[0] & 0x01 || (mac[0] == 0xff && mac[5] == 0xff)) {
#else
    if (1) {
#endif 
        mac[0] = 0x00;
...
int ag7240_enet_initialize(bd_t * bis)
{
    ...
    for (i = 0;i < CFG_AG7240_NMACS;i++) {
        eth_register(dev[i]);
#if(CONFIG_COMMANDS & CFG_CMD_MII)
        miiphy_register(dev[i]->name, ag7240_miiphy_read_ex, ag7240_miiphy_write_ex);
#endif
...
int ag7240_reset_ge(void)
{
	...
	ag7240_reg_wr(ag7240_macs[i], AG7240_MAC_FIFO_CFG_0, 0x1f00);

	ag7240_reg_rmw_set(ag7240_macs[i], AG7240_MAC_FIFO_CFG_4, 0x3ffff);
	/* use cfg5 value from Linux driver - accept broadcasts */
	/*
	* Drop CRC Errors, Pause Frames, Length Error frames, Truncated Frames
	* dribble nibble and rxdv error frames.
	*/
	ag7240_reg_wr(ag7240_macs[i], AG7240_MAC_FIFO_CFG_5, 0x66b80);

	ag7240_reg_wr(ag7240_macs[i], AG7240_MAC_FIFO_CFG_3, 0x1f00140);

}

#if (CONFIG_COMMANDS & CFG_CMD_MII)
int
ag7240_miiphy_read_ex(char *devname, uint32_t phy_addr, uint8_t reg, uint16_t* value)
{
    ...
    *value = ag7240_reg_rd(mac, AG7240_MII_MGMT_STATUS);
    ag7240_reg_wr(mac, AG7240_MII_MGMT_CMD, 0x0);
    udelay(5);
    // printf("ag7240_miiphy_read: DEV:%s PHY:0x%02x REG:0x%02x VAL:0x%08x RET:%d\n", devname, phy_addr, reg, *value, ret);
    return ret;
    // return *value;
}

int
ag7240_miiphy_write_ex(char *devname, uint32_t phy_addr, uint8_t reg, uint16_t data)
{
    ag7240_mac_t *mac = ag7240_name2mac(devname);
    uint16_t      addr  = (phy_addr << AG7240_ADDR_SHIFT) | reg;
    volatile int rddata;

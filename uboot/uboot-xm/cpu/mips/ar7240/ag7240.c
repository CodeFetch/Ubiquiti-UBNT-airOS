...
#undef DEBUG
#define AG7240_LINK_POLLING
#define AG7240_LINK_POLLING_ENABLED_DEFAULT 1
#define AG7240_LINK_POLLING_INTERVAL ( CFG_HZ * 2 )
...
#if (CONFIG_COMMANDS & CFG_CMD_MII)
int mii_ag7240_read(char *devname, unsigned char addr,
			unsigned char reg, unsigned short *value);
int mii_ag7240_write(char *devname, unsigned char addr,
			unsigned char reg, unsigned short value);
#endif
...
#ifdef AG7240_LINK_POLLING
static int enable_link_polling = AG7240_LINK_POLLING_ENABLED_DEFAULT;
static int enable_link_polling_trace = 0;
#endif // #ifdef AG7240_LINK_POLLING
...
#ifdef AG7240_LINK_POLLING
    if(enable_link_polling) {
        static unsigned long last_check_ts = 0;
        // Did the timer interval elapse?
        if (last_check_ts) {
            if (get_timer(last_check_ts) >= AG7240_LINK_POLLING_INTERVAL) {
                if(enable_link_polling_trace) {
                    printf( "\nag7240 link polling [action=%s link=%s packets=%lu elapsed=%lu/%lu sec]\n", 
                            packet_count ? "skip"   : "check",
                            packet_count ? "active" : "inactive", 
                            packet_count, 
                            get_timer(last_check_ts) / CFG_HZ, 
                            AG7240_LINK_POLLING_INTERVAL / CFG_HZ);
                }
                if (!packet_count) {
                    ag7240_check_link( mac );
                }
                packet_count  = 0;
                last_check_ts = get_ticks();
            }
        } else {
            last_check_ts = get_ticks();
        }
...
#else /* use cfg5 value from Linux driver - accept broadcasts */
    /*
...
#ifdef AG7240_LINK_POLLING
    {
        const char* lp = getenv("ag7240_link_polling");
        const char* lpt = getenv("ag7240_link_polling_debug");
        if(NULL != lpt) {
            enable_link_polling_trace = simple_strtoul(lpt, NULL, 16);
        }
        if(NULL != lp) {
            enable_link_polling = simple_strtoul(lp, NULL, 16);
        }
        if(enable_link_polling_trace) {
            printf( "ag7240 link polling is enabled.\n");
        }
    }
#endif // #ifdef AG7240_LINK_POLLING
}
...
static int ag7240_check_link(ag7240_mac_t *mac)
{
    u32 link, duplex, speed, fdx, xmii_config;

    ag7240_phy_link(mac->mac_unit, &link);
    mac->link = link;
    // MT: Moved up to short-circuit earlier.
    if(!mac->link) {
        return 0;
    }
    ...
    ag7240_phy_get_xmii_config(mac->mac_unit, speed, &xmii_config);
    ...
    switch (speed)
    {
       case _1000BASET:
           ...
           if (is_ar7242() && (mac->mac_unit == 0)) 
               ar7240_reg_wr(AR7242_ETH_XMII_CONFIG, xmii_config);
           break;
...
int ag7240_enet_initialize(bd_t * bis)
{
...
#if (CONFIG_COMMANDS & CFG_CMD_MII)
    miiphy_register(dev[i]->name, mii_ag7240_read, mii_ag7240_write);
#endif
    if(!i) {
...
#ifdef DEBUG
    {
	    ...
	    int j;
	    for (j = 0; j < 32; ++j) {
		    id1 = ag7240_miiphy_read(dev[i]->name, j, 2);
		    id2 = ag7240_miiphy_read(dev[i]->name, j, 3);
		    printf("XXX %s %d id1=%04x id2=%04x\n",
				    dev[i]->name, j, id1, id2);
	    }
    }
#endif
...
#define MII_MGMT_IND_NOT_VALID BIT(2)
#define MII_MGMT_IND_SCANNING BIT(1)
#define MII_MGMT_IND_BUSY BIT(0)

extern int eth_verbose;
uint16_t
ag7240_miiphy_read(char *devname, uint32_t phy_addr, uint8_t reg)
{
    ...
    ag7240_reg_wr(mac, AG7240_MII_MGMT_CMD, AG7240_MGMT_CMD_READ);
	    ...
	    {
        udelay(5);
		rddata = ag7240_reg_rd(mac, AG7240_MII_MGMT_IND) & (MII_MGMT_IND_NOT_VALID | MII_MGMT_IND_SCANNING | MII_MGMT_IND_BUSY);
    } while(rddata && --ii);

	if (!ii) {
		printf("%s: failed[0x%x]\n", __func__, rddata);
		return 0; //I guess it will not hurt
	}
...
#if (CONFIG_COMMANDS & CFG_CMD_MII)
int
mii_ag7240_read(char *devname, unsigned char addr, unsigned char reg,
		unsigned short *value) {
	ag7240_mac_t *mac = ag7240_name2mac(devname);
	if (value) {
		if (is_ar7242() && mac->mac_unit == 1) {
			if (addr < 1)
				*value = s26_rd_phy(addr, reg);
			else
				*value = 0xffff;
		} else if (is_ar7240() || is_ar7241()) {
			if (addr < 5)
				*value = s26_rd_phy(addr, reg);
			else
				*value = 0xffff;
		} else {
			*value = ag7240_miiphy_read(devname, addr, reg);
		}
		return 0;
	}
	return -1;
...
int mii_ag7240_write(char *devname, unsigned char addr,
			unsigned char reg, unsigned short value) {
	ag7240_miiphy_write(devname, addr, reg, value);
	return 0;
}

...
#include <../ubnt/include/rsa/rsa.h>
...
static int rsa_get_pub_key(const char *name, RSA **rsap)
{
	...
	*rsap = NULL;
	...
	if (decoded_len<0) {
		printf("Public key decode failure\n");
		ret = -EINVAL;
		goto err_pubkey;
	}
...
int write_key_file(struct rsa_public_key *key, char*outfile)
{
	...
	const char n0_inv_str[] = "uint32_t n0_inv = ";
	const char modulus_str[] = "uint32_t modulus[] = {";
	const char r_squared_str[] = "uint32_t r_squared[] = {";
	const char end_str[] = "};";
	int i;
...
int main(int argc, char *argv[])
{
	...
	if (rsa_preprocess_key(argv[1], &key)) {
		printf("RSA preprocess failure.\n");
		return -1;
	}

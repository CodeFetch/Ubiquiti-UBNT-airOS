...
void plat_dev_init(void)
{
    ...
    if(calData[0] != 0xa55a) {
        /*
        ** Board is either not calibrated, or does not need pci config
        */
        return;
    }
    ...
    */
    if ((is_ar7241() || is_ar7242()))
        ar7240_pci_write_config(&hose, 0, 0x10, 0x1000ffff);
    else
        ar7240_pci_write_config(&hose, 0, 0x10, 0xffff);

    ar7240_pci_write_config(&hose, 0, 0x04, 0x6);
...
#define PCIE_PLL_CONFIG_VALUE 0x00040800

int check_for_radio(void)
{
    unsigned int cmd;

    /* try to avoid reset if PCI is already running */
    if (ar7240_reg_rd(AR7240_PCIE_PLL_CONFIG) == PCIE_PLL_CONFIG_VALUE)
	    return AR7240NoRadio;
    ...
    ar7240_reg_wr(AR7240_PCIE_PLL_CONFIG,PCIE_PLL_CONFIG_VALUE);
    udelay(100);
...
int pci_init_board (void)
{
    ...
    if (((ar7240_reg_rd(AR7240_PCI_LCL_RESET)) & 0x1) == 0x0) {
#ifdef PCI_DEBUG
	/* UBNT: silence this warning on POE Switch - it has no radio */
